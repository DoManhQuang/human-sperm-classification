...
def block_identity(x, filter_block, kernel_size_a=3, kernel_size_b=1, stride=1, activation='relu', padding='same', name="block_identity"):
...
def block_identity_group(x, filter_blocks, activation='relu', padding='same', name="block_identity_group"):
    for i in range(0, len(filter_blocks)):
        ...
        x = block_id
    output_block = Activation(activation, name=name)(x)
    return output_block
...
# model = created_model_hsc_01(input_shape=(40, 40, 3), number_class=2, activation_dense='softmax')
